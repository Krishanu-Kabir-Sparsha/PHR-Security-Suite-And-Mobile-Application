...
from .install_guard import in_module_loading
...
class ResUsers(models.Model):
    ...
    def write(self, vals):
        ...
        Bypassed when the environment carries ``plaza_bypass_grant_check``, and
        whenever Odoo is installing or upgrading a module. Group assignment
        during install has no acting human to justify it, and stock modules
        grant their own groups from their own data files -- ``purchase`` hands
        ``group_purchase_manager`` to a stock user in
        ``purchase/security/purchase_security.xml``. Refusing that aborts the
        install with a ParseError while protecting nothing, because nobody is
        escalating anything: it is the module declaring its own role.

        See ``install_guard.in_module_loading`` for why the test-run case is
        deliberately excluded from the bypass.
        """
        if (
            "groups_id" not in vals
            or self.env.context.get("plaza_bypass_grant_check")
