...
# License AGPL-3.0 or later (https://www.gnu.org/licenses/agpl).
{
    ...
    # 18.0.1.2.1 - the shipped auditlog rules are now subscribed idempotently,
    # on upgrade as well as install, and the nightly chain check reports any
    # rule found dormant. A draft rule captures nothing while every screen
    # still looks healthy, so it must not depend on a hook that only ever runs
    # at install time.
    "version": "18.0.1.2.1",
    "category": "Security",
    "license": "AGPL-3",
