...
class LockerEntry(models.Model):
    ...
    # ------------------------------------------------------------------
    # Keeping capture switched on
    # ------------------------------------------------------------------
    @api.model
    def _locker_rules(self):
        """The auditlog rules this module ships, or None if auditlog is absent.

        Found by external ID rather than by the "Locker:" name prefix, so a
        renamed rule is still recognised and an unrelated rule that happens to
        be named similarly is not adopted.
        """
        ...
        data = (
            ...
            .sudo()
            .search([("module", "=", "sec_audit_locker"), ("model", "=", "auditlog.rule")])
        )
        return self.env["auditlog.rule"].sudo().browse(data.mapped("res_id")).exists()

    @api.model
    def ensure_rules_subscribed(self, alert=True):
        """Subscribe any Locker rule that is not subscribed. Idempotent.

        This exists because a Locker rule sitting in ``draft`` is the quietest
        possible failure in the whole suite. OCA auditlog only patches the ORM
        for *subscribed* rules, so a dormant rule means no auditlog rows, which
        means no Locker entries, which means ``verify_chain()`` cheerfully
        reports an intact chain -- of zero entries. Everything looks healthy
        and nothing is being recorded.

        It is not hypothetical: the rules ship in ``draft`` and are subscribed
        by ``post_init_hook``, which Odoo runs on install and *not* on upgrade.
        Any install that gained the auditlog dependency later therefore has the
        rules present and dormant.

        Both actions are taken deliberately. The rules are re-subscribed,
        because BRD FR-4.2 does not treat the audit trail as optional, and they
        are *also* reported, because silently switching capture back on would
        hide the fact that it was off -- including if someone turned it off on
        purpose. Fixing it and saying so is the only combination that leaves
        both the control and the record intact.

        :param alert: raise an anomaly for rules that had to be fixed. False
            during install, where the rules have simply never been subscribed
            yet and nothing has happened worth reporting.
        :return: the rules that were dormant and have been subscribed.
        """
        rules = self._locker_rules()
        if rules is None:
            _logger.warning(
                "OCA auditlog is not installed, so the Audit Locker captures "
                "nothing. No rules to subscribe."
            )
            return None
        if not rules:
            _logger.warning(
                "No Audit Locker auditlog rules found. Expected the rules from "
                "data/auditlog_rules.xml; the Locker will capture nothing."
            )
            return rules

        dormant = rules.filtered(lambda r: r.state != "subscribed")
        if not dormant:
            return rules.browse()
        ...
        if fixed:
            _logger.warning(
                "Audit Locker capture was not active for %s rule(s); "
                "subscribed: %s",
                len(fixed),
        ...
        if alert and fixed:
            self.env["sec.anomaly.mixin"]._raise_anomaly(
                alert_type="other",
                name=_("Audit Locker capture was not active"),
                reason=_(
                    "%(count)s Locker rule(s) were not subscribed, so nothing "
                    "was being recorded for them: %(names)s. They have been "
                    "re-subscribed. Anything that happened to those models "
                    "while the rules were dormant left no audit trail and "
                    "cannot be reconstructed.",
                    count=len(fixed),
                    names=", ".join(fixed.mapped("name")),
                ),
                severity="critical",
            )
        return fixed
    ...
    def cron_verify_chain(self):
        """Daily: confirm capture is on, then verify the chain.

        Subscription is checked first on purpose. Verifying the chain before
        knowing whether anything is being captured answers the wrong question:
        an empty chain is always intact.
        """
        self.ensure_rules_subscribed()
...
def post_init_hook(env):
    ...
    # Scoped to this module's own rules, and alert=False: on a fresh install
    # they have simply never been subscribed yet. The previous version
    # subscribed *every* unsubscribed auditlog rule in the database, which
    # silently commandeered rules belonging to somebody else.
    env["audit.locker.entry"].ensure_rules_subscribed(alert=False)
