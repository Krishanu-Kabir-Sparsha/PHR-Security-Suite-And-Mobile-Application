...
class AnomalyMixin(models.AbstractModel):
    ...
    def _raise_anomaly_out_of_band(
        self,
        alert_type,
        name,
        reason,
        ...
        """Write an alert on a SEPARATE cursor, so a rollback cannot erase it.

        Use this -- never plain ``_raise_anomaly`` -- whenever the caller is
        about to ``raise``. An exception rolls its transaction back, and an
        alert written on that same cursor goes with it, so the record of a
        refused action is destroyed by the very refusal it was recording. The
        failure is silent and total: the code reads correctly, the alert is
        created, and nothing is ever in the table.

        The distinction is not academic. Every alert raised alongside a refusal
        is, by definition, one somebody tried to do something they could not
        do -- which is precisely the population an anomaly report exists to
        show. Losing exactly those is losing the ones that matter.

        ``record`` is not accepted. The new cursor is a different transaction,
        and a record created in the caller's uncommitted one would not be
        visible from it; pass what you need into ``reason`` instead. Where the
        record is already saved, ``self`` is stamped into ``source_ref``.

        Never raises, for the same reason ``_raise_anomaly`` never does: a
        fault in alerting must not mask the security decision that triggered
        it.
        """
        source_ref = (
        ...
        acting_user = user or self.env.user
        try:
            with self.pool.cursor() as new_cr:
                ...
                env["sec.anomaly.mixin"]._raise_anomaly(
                    ...
                    reason=reason,
                    severity=severity,
                    user=acting_user.with_env(env),
                    source_ref=source_ref,
                )
        except Exception:  # noqa: BLE001 - alerting must never break the caller
            _logger.exception(
                "Failed to write out-of-band anomaly alert (type=%s, name=%s). "
                "The security decision that triggered it still stands.",
                alert_type,
                name,
            )
