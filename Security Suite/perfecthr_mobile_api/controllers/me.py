...
from .common import authenticated, fail, ok, request_employee
...
class MobileMe(http.Controller):
    ...
    def _attendance(self, employee):
        """Today's attendance, from the one shared definition.

        This used to compute its own answer, and `attendance.py` computed
        another, and the two applied slightly different rules -- so the Home
        screen and the Attendance screen could describe the same morning
        differently. Both now read
        ``perfecthr.mobile.checkin.day_state``.

        The two labels appended here are presentation the home card needs and
        the attendance payload carries at its top level instead.
        """
        state = (
            request.env["perfecthr.mobile.checkin"].sudo().day_state(employee)
        )
        return {
            **state,
