...
from odoo.addons.sec_plaza_rbac.models.install_guard import in_module_loading
...
class RecordFreezeMixin(models.AbstractModel):
    ...
    def _freeze_checks_suspended(self):
        """True when the freeze must stand down for this operation.

        Two cases, and only two. The explicit ``sec_freeze_install_mode``
        context key, which a caller sets deliberately; and Odoo installing or
        upgrading a module, where the writes are Odoo replaying its own data
        and demo files rather than a user editing a confirmed document. The
        second case previously had no way to signal itself -- nothing in Odoo
        has ever set ``sec_freeze_install_mode`` -- so installing ``account``
        or ``purchase`` alongside a frozen stream aborted the registry build.

        Test runs are deliberately *not* suspended even though they happen
        inside the same registry build -- see ``install_guard.in_module_loading``.
        """
        return bool(
            self.env.context.get("sec_freeze_install_mode")
            or in_module_loading(self.env)
        )
    ...
    def create(self, vals_list):
        ...
        """
        if not self._freeze_checks_suspended():
    ...
    def unlink(self):
        ...
        rule = self._freeze_rule()
        if rule and rule.block_unlink and not suspended:
            frozen = self._filter_frozen(rule)
