...
``py_webauthn`` 2.2.0. Enrolment now completes; approvals can be confirmed.
...
``py_webauthn`` 2.2.0 must be installed on the Odoo server (see "Why 2.2.0" below)::
...
Install it into the *server's* Python, not a user account's::

    "C:\Program Files\Odoo 18\python\python.exe" -m pip install webauthn==2.2.0

run from an **elevated** shell. Without elevation pip prints "Defaulting to user
installation because normal site-packages is not writeable" and installs into
``%APPDATA%\Python``, which the service account (``NT AUTHORITY\LocalService``)
cannot read. The module then still reports the dependency as missing, with
nothing in the log to say why.

Why 2.2.0 and not the latest
============================

Odoo 18 pins ``cryptography==42.0.8`` and ``pyopenssl==24.1.0``, and pyOpenSSL
24.1.0 caps ``cryptography<43``. py_webauthn moved past that pin some time ago:

============  ==========================  ==================================
Version       Requires                    Usable here
============  ==========================  ==================================
2.2.0         cryptography (unpinned)     **yes** — what we install
2.5.0-2.7.x   cryptography>=43.0.3        no, breaks Odoo's pyOpenSSL pin
2.8.0         cryptography>=46.0.0        no
3.0.0         cryptography>=49.0.0        no
============  ==========================  ==================================

Taking 3.0.0 means dragging Odoo's whole TLS stack forward — cryptography 42 to
50, pyOpenSSL 24 to 26 — to gain nothing this module uses. The only API we lose
below 2.5.0 is the ``require_user_presence`` keyword, and losing it costs
nothing: in 2.2.0 the presence check is *unconditional*, raising on
``not auth_data.flags.up`` in both ceremonies with no way to disable it. See the
comment at the registration call in ``models/webauthn_verify.py``.

If you later need a newer py_webauthn, upgrade Odoo's cryptography and pyOpenSSL
together and deliberately, as their own change — not as a side effect of
